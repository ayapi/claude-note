...
(本文側には何も現れない)。OneNote 自身の文字起こし (`recognizedText`) は日本語だと
空白しか返らないので当てにせず、**インクを画像にして Claude に読ませる**。
そのときだけ `titleInkPromptLine` が開始プロンプトの先頭に足される。

描画には注意がいる。1 画ごとに `one:InkWord` に分かれるが、**ISF の座標は全画で
1 つの空間を共有している** (要素の `inkOriginX` がその差を表す)。要素の
`x`/`width` に合わせて 1 画ずつ拡縮すると、倍率が 0.04〜4.8 のようにばらついて
字が崩れる。ISF の実寸から矩形を起こし、全画を同じ倍率で並べること。

`ClaudeNote.exe --title-scan` で `one:Title` の生 XML、各画の実寸と指定矩形、
倍率が出る。`--title-scan section` はセクション内の全ページを一覧する。
